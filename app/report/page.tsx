"use client";
import { useState, useEffect, useRef } from "react";
import Link from "next/link";
import { ChevronLeft, PhoneCall, MapPin, Camera, Navigation, Play, Square, Activity, AlertTriangle } from "lucide-react";
import { mockSensorData } from "@/lib/mock-websocket";

type SensorReadingLocal = ReturnType<typeof mockSensorData>;

export default function ReportPage() {
  const [tripActive, setTripActive] = useState(false);
  const [sensorData, setSensorData] = useState<SensorReadingLocal[]>([]);
  const [anomalyFlash, setAnomalyFlash] = useState(false);
  const [sosPressed, setSosPressed] = useState(false);
  const [form, setForm] = useState({ type: "pothole", description: "", gps: "" });
  const [submitted, setSubmitted] = useState(false);
  const [gpsLoading, setGpsLoading] = useState(false);
  const tripRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const chartData = sensorData.slice(-40);

  useEffect(() => {
    if (tripActive) {
      tripRef.current = setInterval(() => {
        const reading = mockSensorData();
        setSensorData((prev) => [...prev.slice(-200), reading]);
        if (reading.anomalyDetected) {
          setAnomalyFlash(true);
          setTimeout(() => setAnomalyFlash(false), 800);
        }
      }, 200);
    } else {
      if (tripRef.current) clearInterval(tripRef.current);
    }
    return () => { if (tripRef.current) clearInterval(tripRef.current); };
  }, [tripActive]);

  function detectGPS() {
    setGpsLoading(true);
    setTimeout(() => {
      setForm((f) => ({ ...f, gps: "12.9784° N, 77.6408° E (Whitefield)" }));
      setGpsLoading(false);
    }, 1200);
  }

  function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    setSubmitted(true);
    setTimeout(() => setSubmitted(false), 3000);
  }

  const latest = sensorData[sensorData.length - 1];
  const maxZ = Math.abs(Math.min(...chartData.map((d) => d.zAccel), -1));

  return (
    <div className="min-h-screen bg-[#0a0a0f] text-[#e2e8f0]">
      <header className="border-b border-[#2a2a3a] px-4 py-3 flex items-center gap-4">
        <Link href="/" className="flex items-center gap-1 text-[#6b7280] hover:text-[#e2e8f0] transition-colors text-sm">
          <ChevronLeft size={14} /> Back
        </Link>
        <span className="font-semibold text-sm">Report & Assistance</span>
      </header>

      <div className="max-w-lg mx-auto px-4 py-6 space-y-6">
        {/* SOS Button */}
        <div className={`rounded-2xl border-2 transition-all p-6 text-center ${sosPressed ? "border-red-400 bg-red-500/10" : "border-red-500/40 bg-[#111118]"}`}>
          <p className="text-[#6b7280] text-sm mb-4">Vehicle breakdown or road emergency?</p>
          <button
            onMouseDown={() => setSosPressed(true)}
            onMouseUp={() => setSosPressed(false)}
            onTouchStart={() => setSosPressed(true)}
            onTouchEnd={() => setSosPressed(false)}
            className={`w-32 h-32 rounded-full font-bold text-lg transition-all flex flex-col items-center justify-center gap-2 mx-auto border-4 ${sosPressed ? "bg-red-600 border-red-400 scale-95" : "bg-red-500 border-red-600 hover:bg-red-600 active:scale-95"} text-white shadow-lg shadow-red-500/30`}
          >
            <PhoneCall size={28} />
            <span className="text-sm">EMERGENCY</span>
          </button>
          <p className="text-[#6b7280] text-xs mt-4">Hold to alert nearest BBMP crew + emergency services</p>
        </div>

        {/* Trip Sensor Monitor */}
        <div className={`rounded-xl border p-4 ${anomalyFlash ? "border-amber-400 bg-amber-500/5" : "border-[#2a2a3a] bg-[#111118]"} transition-all`}>
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-2">
              <Activity size={16} className={tripActive ? "text-green-400" : "text-[#6b7280]"} />
              <span className="font-semibold text-sm">Trip Sensor Monitor</span>
            </div>
            <button
              onClick={() => setTripActive((v) => !v)}
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${tripActive ? "bg-red-500/20 text-red-400 border border-red-500/30 hover:bg-red-500/30" : "bg-green-500/20 text-green-400 border border-green-500/30 hover:bg-green-500/30"}`}
            >
              {tripActive ? <><Square size={12} fill="currentColor" /> Stop Trip</> : <><Play size={12} fill="currentColor" /> Start Trip</>}
            </button>
          </div>

          {tripActive ? (
            <>
              {anomalyFlash && (
                <div className="mb-3 bg-amber-500/20 border border-amber-500/30 rounded-lg px-3 py-2 text-amber-400 text-sm flex items-center gap-2">
                  <AlertTriangle size={14} />
                  Anomaly detected! Z-axis spike = {latest?.zAccel.toFixed(2)}g
                </div>
              )}
              <div className="space-y-3">
                {[
                  { label: "Z-Accel (g)", value: latest?.zAccel.toFixed(3) ?? "—", color: Math.abs(latest?.zAccel ?? 0) > 2 ? "text-red-400" : "text-green-400" },
                  { label: "Gyro X (°/s)", value: latest?.gyroX.toFixed(3) ?? "—", color: Math.abs(latest?.gyroX ?? 0) > 1 ? "text-amber-400" : "text-blue-400" },
                  { label: "Gyro Y (°/s)", value: latest?.gyroY.toFixed(3) ?? "—", color: Math.abs(latest?.gyroY ?? 0) > 1 ? "text-amber-400" : "text-blue-400" },
                ].map((m) => (
                  <div key={m.label} className="flex items-center gap-3">
                    <span className="text-[#6b7280] text-xs w-24">{m.label}</span>
                    <span className={`font-mono text-sm font-bold ${m.color}`}>{m.value}</span>
                  </div>
                ))}
                {/* Mini waveform */}
                <div className="mt-3 h-16 flex items-end gap-0.5 overflow-hidden rounded bg-[#0a0a0f] p-2">
                  {chartData.map((d, i) => {
                    const h = Math.min(100, (Math.abs(d.zAccel) / maxZ) * 100);
                    return (
                      <div
                        key={i}
                        className={`flex-1 rounded-sm transition-all ${d.anomalyDetected ? "bg-red-400" : "bg-blue-500/60"}`}
                        style={{ height: `${h}%` }}
                      />
                    );
                  })}
                </div>
                <p className="text-[#6b7280] text-xs">Accelerometer waveform · Red spikes = detected anomalies</p>
              </div>
            </>
          ) : (
            <p className="text-[#6b7280] text-sm text-center py-4">
              Start a trip to begin collecting Z-axis accelerometer and gyroscope data for pothole detection.
            </p>
          )}
        </div>

        {/* Manual Report Form */}
        <div className="rounded-xl border border-[#2a2a3a] bg-[#111118] p-4">
          <h2 className="font-semibold text-sm mb-4 flex items-center gap-2">
            <MapPin size={14} className="text-blue-400" />
            Report a Road Issue
          </h2>
          {submitted ? (
            <div className="text-center py-6 text-green-400">
              <div className="text-3xl mb-2">✓</div>
              <p className="font-semibold">Report submitted!</p>
              <p className="text-[#6b7280] text-sm mt-1">BBMP has been notified.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="text-xs text-[#6b7280] mb-1.5 block">Issue Type</label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { value: "pothole", label: "🕳️ Pothole" },
                    { value: "speedbreaker", label: "⚠️ Speed Breaker" },
                    { value: "breakdown", label: "🚗 Breakdown" },
                  ].map((opt) => (
                    <button
                      key={opt.value}
                      type="button"
                      onClick={() => setForm((f) => ({ ...f, type: opt.value }))}
                      className={`py-2 px-3 rounded-lg text-xs font-medium border transition-all ${form.type === opt.value ? "border-blue-500 bg-blue-500/20 text-blue-400" : "border-[#2a2a3a] text-[#6b7280] hover:border-[#3a3a4a]"}`}
                    >
                      {opt.label}
                    </button>
                  ))}
                </div>
              </div>

              <div>
                <label className="text-xs text-[#6b7280] mb-1.5 block">GPS Location</label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={form.gps}
                    readOnly
                    placeholder="Tap to auto-detect..."
                    className="flex-1 bg-[#0a0a0f] border border-[#2a2a3a] rounded-lg px-3 py-2 text-sm text-[#e2e8f0] placeholder-[#6b7280] outline-none"
                  />
                  <button
                    type="button"
                    onClick={detectGPS}
                    disabled={gpsLoading}
                    className="flex items-center gap-1.5 px-3 py-2 bg-blue-500/20 border border-blue-500/30 text-blue-400 rounded-lg text-xs hover:bg-blue-500/30 transition-colors disabled:opacity-50"
                  >
                    <Navigation size={12} />
                    {gpsLoading ? "..." : "GPS"}
                  </button>
                </div>
              </div>

              <div>
                <label className="text-xs text-[#6b7280] mb-1.5 block">Description</label>
                <textarea
                  value={form.description}
                  onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))}
                  placeholder="Describe the issue..."
                  rows={3}
                  className="w-full bg-[#0a0a0f] border border-[#2a2a3a] rounded-lg px-3 py-2 text-sm text-[#e2e8f0] placeholder-[#6b7280] outline-none focus:border-blue-500/50 resize-none"
                />
              </div>

              <div>
                <label className="text-xs text-[#6b7280] mb-1.5 block">Photo (optional)</label>
                <label className="flex items-center justify-center gap-2 border border-dashed border-[#2a2a3a] rounded-lg py-6 cursor-pointer hover:border-blue-500/40 hover:bg-blue-500/5 transition-all text-[#6b7280] text-sm">
                  <Camera size={16} />
                  Tap to upload photo
                  <input type="file" accept="image/*" className="hidden" capture="environment" />
                </label>
              </div>

              <button
                type="submit"
                className="w-full bg-blue-500 hover:bg-blue-600 text-white font-semibold py-3 rounded-lg transition-colors text-sm"
              >
                Submit Report
              </button>
            </form>
          )}
        </div>
      </div>
    </div>
  );
}
